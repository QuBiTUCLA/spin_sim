...
for kappa = 1%1:10:a(2) %loop over waiting times
...
% 
%  figure(1);
%  plot(karray,log10(poi_distrB),'*r',karray,log10(poi_distrD),'sk');
%  title('simulated poissonian distr for Bright state (red) and Dark (black)')
%  xlabel('Nb photons')
%  ylabel('Log10 of occurences')
 
% compare to seen before
% Now I want the integrated PL collected

int_PL_zero_g = sum(NbarB(1:1:B_array_position_end_det1));
int_PL_m_one_g = sum(NbarD(1:1:D_array_position_end_det1));
...
% single shot with threshold
thresh = 0;
index = thresh + 1; % index is of k == thresh
errorB = sum(poi_distrB(1:index));
errorD = sum(poi_distrD(index+1:end));
avg_error_singleshot = (errorB + errorD)/2;

%errorB
%errorD
%avg_error_singleshot

%with averaging: NN is the number of reps per point
NN = repsperpt;
%NN

% excluding fluctuations in mw, laser, moving NV, difference in processes
% in averaging x reps per point
% formula = 1.96*sqrt(a*(1-a)/n), a is amplitude % from http://en.wikipedia.org/wiki/Binomial_proportion_confidence_interval
% since we're using the avg error, amplitude a = 1/2
averaged_final_error_singleshot = avg_error_singleshot*1.96*sqrt((1/2)*(1-1/2))/sqrt(NN);

%averaged_final_error_singleshot
...
for q = B_array_position_begin_det2:1:length(res{auxi,kappa}.init0.T)
  ...
  NbarBpi(q) = (res{auxi,kappa}.init0.pop_zero_e(q) + res{auxi,kappa}.init0.pop_mone_e(q) + res{auxi,kappa}.init0.pop_one_e(q))*(77e6)*tstep + 1.5e6*(res{auxi,kappa}.init0.pop_mone_e(q) + res{auxi,kappa}.init0.pop_one_e(q))*tstep + 1.5e6*(2*res{auxi,kappa}.init0.pop_zero_e(q))*tstep;
end
%Dark
for q = D_array_position_begin_det2:1:length(res{auxi,kappa}.init1.T)
  ...
  NbarDpi(q) = (res{auxi,kappa}.init1.pop_zero_e(q) + res{auxi,kappa}.init1.pop_mone_e(q) + res{auxi,kappa}.init1.pop_one_e(q))*(77e6)*tstep + 1.5e6*(res{auxi,kappa}.init1.pop_mone_e(q) + res{auxi,kappa}.init1.pop_one_e(q))*tstep + 1.5e6*(2*res{auxi,kappa}.init1.pop_zero_e(q))*tstep;
end
...
% figure(2);
% plot(karray,log10(poi_distrBpi),'*r',karray,log10(poi_distrDpi),'sk');
% title('simulated poissonian distr for Bright state (red) and Dark (black) for second detection')
% xlabel('Nb photons')
% ylabel('Log10 of occurences')

% pi detection with threshold
errorpiB = sum(poi_distrBpi(1:index));
errorpiD = sum(poi_distrDpi(index+1:end));
avg_error_pi = (errorpiB + errorpiD)/2;

%errorpiB
%errorpiD
%avg_error_pi

% Multiplying the errors to compare
FinalerrorBpi = errorB*errorpiD;
FinalerrorDpi = errorD*errorpiB;

%estimate percentage of used statistics, by using decision tree

usedB = 1 - errorB*(1-errorpiD) - (1-errorB)*errorpiD;
usedD = 1 - (1-errorD)*errorpiB - errorD*(1-errorpiB);

%usedB
%usedD

averagedFinalerrorpi = ((FinalerrorBpi+FinalerrorDpi)/2)*1.96*sqrt((1/2)*(1-1/2))/sqrt(NN*(usedB+usedD)/2);

%averagedFinalerrorpi
