...
a = 2.06e-6; %    0.58*1e-9*54*[65, 85, 87, 117, 164];
b = 2.05e-6; %          0.58*1e-9*54*[50, 65, 77, 107, 154];
c = 2.07e-6;          %0.58*1e-9*54*[70, 95, 97, 127, 174];
...
experrorSIG = Scan.ExperimentDataError{1}{1}/sqrt(100000);  %doing moving avg for repetitions as well %%%%%%%%%%%%%% SHOULD THIS BE CONSIDERED AT ALL???
...
plot(w,rabinorm)
hold on
myfun = @(p, w) p(1) + p(2)*cos(2*pi*p(5)*(p(6)-w) + pi*p(8)) + p(3)*cos(2*pi*p(5)*(p(6)+p(7)-w) + pi*p(8)) + p(4)*cos(2*pi*p(5)*(p(6)-p(7)-w)+ pi*p(8)); 
...
%p(5) osci frequency
init(5) = 2.061e-6;% a(aux);
Lo(5) = 2.061e-6; %b(aux);
Hi(5) = 2.061e-6; %c(aux);

%p(6) reson frequency
init(6) = 3.1575e9;
Lo(6) =  3.1575e9;%3.1565e9;
Hi(6) =  3.1575e9;%3.1575e9;
