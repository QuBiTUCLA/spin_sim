...
IntT(aux) = 2*Scan.Variable_values{8}.value*Scan.Variable_values{6}.value;
...
experrorONE = Scan.ExperimentDataError{1}{3}/sqrt(100000); 
...
experrorSIG = stdd{1}/sqrt(100000);
experrorONE = stdd{3}/sqrt(100000);
experrorZERO = stdd{4}/sqrt(100000);
...
LB =    [0.01,    b(aux),      3.155e9,             0.01,   0, 1.9e6,   0      0];
UB =    [0.4,     c(aux),      3.159e9,              0.7,   0, 2.4e6,   0,     0];
if aux == 2
pinit = [0.2,     a(aux),      3.157e9,   mean(rabinorm),   0, 2.1e6,   0,     0];
LB =    [0.01,    b(aux),      3.1568e9,             0.01,   0, 1.9e6,   0      0];
UB =    [0.4,     c(aux),      3.1572e9,              0.7,   0, 2.4e6,   0,     0];
end
...
% pinit = [0.2,     a(aux),      3.157e9,   mean(rabinorm),   0, 2.1e6,   0,     0];
% LB =    [0.01,    b(aux),      3.1568e9,             0.01,   0, 1.9e6,   0      0];
% UB =    [0.4,     c(aux),      3.158e9,              0.7,   0, 2.4e6,   0,     0];
% end
% 
% if aux == 5
% pinit = [0.18,     2*pi*3.67e-6,      3.157e9,   mean(rabinorm),   0, 2.1e6,   0,     0];
% LB =    [0.01,    b(aux),            3.1568e9,             0.01,   0, 1.9e6,   0      0];
% UB =    [0.4,    2*pi*4.5e-6 ,      3.1573e9,              0.9,   0, 2.4e6,   0,     0];
% end

[pbest,delta_p]=easyfit(w, rabinorm, pinit, myfun, LB, UB);
hold off

n0 = mean(zeroref)*1000*100e-9;
n1 = mean(oneref)*1000*100e-9; 
