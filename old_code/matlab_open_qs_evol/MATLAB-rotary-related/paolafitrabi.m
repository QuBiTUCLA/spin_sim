...
mva=8;
for k=1:4
Scan.ExperimentData{1}{k}=Scan.ExperimentData{1}{k}(30-mva+1:end-65);
end
...
for n=mva:length(rabinorm)
rabinorma(n-mva/2)=mean(rabinorm(n-mva+1:n));
end
rabinorma=rabinorma(mva/2:end);
rabinorm=rabinorma;
% figure(13)
% plot(w,rabinorm)
...
hold on
%fit decaying sin
myfun = @(p, w) p(7) - p(1)*(p(4)^2./((w - p(6)).^2 + p(4)^2)).*sin(pi*9.184e-6*sqrt(p(4)^2 + (p(6) - w).^2)).^2 +...
                     - p(2)*(p(4)^2./((w - p(6) + p(5)).^2 + p(4)^2)).*sin(pi*9.184e-6*sqrt(p(4)^2 + (p(6) - w + p(5)).^2)).^2+...
                     - p(3)*(p(4)^2./((w - p(6) - p(5)).^2 + p(4)^2)).*sin(pi*9.184e-6*sqrt(p(4)^2 + (p(6) - w - p(5)).^2)).^2;
...
pinit = [0.2, 0.2, 0.2, 17.8e6, 2.2e6, 3.1565e9, .75];
% bounds for fitting parameters 
LB =    [0.1, 0.1, 0.1,  17e6,   1.9e6, 3.1560e9, 0.5];
UB =    [0.35, 0.35, 0.35, 19e6,   2.3e6, 3.1570e9, 1];
