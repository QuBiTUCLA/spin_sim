...
plot(diff(Sigma),'r')
hold on
plot(tsmovavg(diff(Sigma),'s',lagpts),'b')

diffe = tsmovavg(diff(Sigma),'s',lagpts)/(w(2)-w(1));  %%%!!!!! MOV AVG HERE SMOOTHENS DERIVATIVE DS/DW
...
%%% ????? /2 in exp? 
minfreqth = @(time,C,angle,T2) 1/C./time*angle/2/sin(angle/2)^2.*exp((time/(T2*(angle)/2/sin(angle/2))).^2)/2/pi/sqrt(40*100000); 
...
plot(tempi,minfreqth(tempi,6.4755e-3,pi*1.0868,5.4e-6),'m')

%%%%% Sensitivities
figure(122)
hold on
gyro = 1.760859708e11; % s-1 T-1
plot(IntT,deltaw.*sqrt(IntT*40*100000)/gyro*1e9*(2*pi)^(3/2),'-*k')
plot(tempi,deltawid.*sqrt(tempi*40*100000)/gyro*1e9*(2*pi)^(3/2),'r')
plot(tempi,minfreqth(tempi,6.4755e-3,pi*1.0868,5.4e-6).*sqrt(tempi*40*100000)/gyro*1e9*(2*pi)^(3/2),'m')
